...
class BranchController extends Controller
{
    public function getBranches()
    {
        $branches = Branch::where('active', ActiveStatus::Active->value)
            ->get(['id', 'title', 'address', 'google_map_url', 'working_period_group_id', 'is_delivery_available', 'is_pickup_available', 'is_busy', 'order_time_from', 'order_time_to'])
            ->map(function($branch) {
                return [
                    'id'                    => $branch->id,
                    'title'                 => $branch->title,
                    ...
                    'current_period_ends_at'=> $branch->currentPeriodEndsAt()?->toIso8601String(),
    ...
    {
        $locations = BranchLocation::with('priceGroup')->where('branch_id', $branchId)->where('active', true)->get();
